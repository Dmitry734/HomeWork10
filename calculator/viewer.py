...
def get_value(flag):
    ...
    if flag == 0:
        ...
        while (chck):
            ...
            if numb_type == 'r':
                numb_value = input(
                    "real number value >> (using '.') or ([int part]space[numerat]/[denominat]) = ")
        ...
        numb_value = str(complex(rc()*random.randint(1, 10),
                         rc()*random.randint(1, 10)))[1:-1]
    return numb_value
...
def get_oper(flag):
    ...
    if flag == 0:
        ...
        numb_oper = op[random.randint(0, 3)]
    return numb_oper
...
def view_res(flag, res):
    if flag == 0:
        ...
        while (chck):
            chck = True
            res_path = input(
                'output the result to (c - console, f - local file): ')
            if res_path in ('c', 'f'):
                chck = False
    else:
        res_path = 'f'
